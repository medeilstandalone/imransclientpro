...
public var selHealthConditions:ArrayList=new ArrayList();//selHealthConditions->Selected Patient HealthConditions
public class HealthConditions {
    var chkbox: CheckBox[]= CheckBox {};
    var log: Logger = Logger.getLogger(HealthConditions.class, "HeathConditions");
    var healthConditions: String[];
    def CustomAlert = CustomAlert {};
    
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:main
    def __layoutInfo_lblHealthCondition: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
    }
    public-read def lblHealthCondition: javafx.scene.control.Label = javafx.scene.control.Label {
        visible: false
        layoutX: bind lbloption.layoutX
        layoutY: bind hbox2.layoutY-27
        layoutInfo: __layoutInfo_lblHealthCondition
        text: "Label"
    }
    
    def __layoutInfo_txtHealthCondition: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: bind rectangle.width-110
    }
    public-read def txtHealthCondition: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        visible: false
        layoutX: bind lblHealthCondition.layoutX+100
        layoutY: bind hbox2.layoutY-27
        layoutInfo: __layoutInfo_txtHealthCondition
    }
    ...
    public-read def rectScrollBg: javafx.scene.shape.Rectangle = javafx.scene.shape.Rectangle {
        ...
        height: bind (healthConditions.size()*21)+10
    }
    ...
    def __layoutInfo_lblHealthConditions: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: 175.0
        height: 26.0
    }
    public-read def lblHealthConditions: javafx.scene.control.Label = javafx.scene.control.Label {
        layoutX: 40.0
        layoutY: 0.0
        layoutInfo: __layoutInfo_lblHealthConditions
    ...
    public-read def panelheader: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        ...
        content: [ rectangle, RectTitle, lblHealthConditions, imageView2, imgClose, lbloption, lblHealthCondition, txtHealthCondition, scrollView, vbox3, hbox2, panelPopUp, panelAlert, ]
    }
                ...
                keyFrames: [
                    javafx.animation.KeyFrame {
                        ...
                        action: function() {
                            ...
                            lblHealthCondition.visible = true;
                            lblHealthCondition.managed = true;
                            __layoutInfo_lblHealthCondition.width = 120.0;
                            lblHealthCondition.text = "Enter new HC :";
                            lblHealthCondition.font = Arial_Bold_12;
                            lblHealthCondition.textFill = DarkGray;
                            txtHealthCondition.visible = true;
                            txtHealthCondition.onKeyPressed = txtHealthConditionOnKeyPressedAtShown;
                            txtHealthCondition.multiline = false;
    ...
    try{
            selHealthConditions.clear();
            for (i in [0..healthConditions.size()-1])
            {
                if(chkbox[i].selected==true)
                {
                    selHealthConditions.add(chkbox[i].text);
                }
            }
            if(selHealthConditions.size()==0)
            {
                FXinfo("Please Select any one Health Condition to Add !");
            }
            else
            {
                def pd=PatientDetails{};
                pd.patientHealthConditions(selHealthConditions);
                Hide();
            }
       }
        catch (e: Exception) {
            var msg: String = "Class : HealthConditions  Method : btnOKActionAtShown() = {e.getMessage()}";
            log.debug(msg);
        }
     }
    
    function txtHealthConditionOnKeyPressedAtShown(event: javafx.scene.input.KeyEvent): Void {
    ...
    function btnAddNewActionAtShown(): Void {
            ...
            healthCondition = txtHealthCondition.rawText.trim();
            if (healthCondition == null or healthCondition == "") {
                FXinfo("Please Enter a HealthCondition");
                } 
            else {
                var patientDetailsController: Customer = RegistryFactory.getClientStub(RegistryConstants.Customer) as Customer;
                var a=patientDetailsController.addHealthCondition(healthCondition);
                if(a==1)
                {
                    FXinfo("HealthCondition Already Exists !");
                    txtHealthCondition.requestFocus();
                }
                else
                {
                    Hide();
    ...
    function FXinfo(msg: String): Void {
        CustomAlert.ShowInfo("Add HealthCondition(s)", msg);
        showAlertbox();
    }
    ...
    public function initShow(): Void {
        ...
        fadePanel.rate = 1.0;
        fadePanel.play();        
        vbox3.content = for (i in [0..healthConditions.size() - 1])
        chkbox[i]=CheckBox{
                    visible: true
                    cursor: javafx.scene.Cursor.HAND
                    layoutX: 0.0
                    layoutY: 0.0
                    text: healthConditions[i]
                    font: Arial_12
           ...
           if(selHealthConditions.size()>0){
           for(j in [0..selHealthConditions.size()-1]){
             for (i in [0..chkbox.size()-1]){
               if(chkbox[i].text==selHealthConditions.get(j)){
                   chkbox[i].selected=true;
               }
             }
           }
         }
        panelheader.content = [rectangle, RectTitle, lblHealthConditions, imageView2, imgClose, lbloption, scrollView, vbox3, hbox2, panelPopUp, panelAlert,lblHealthCondition,txtHealthCondition];
    ...
    init  {
            var patientDetailsController: Customer = RegistryFactory.getClientStub(RegistryConstants.Customer) as Customer;
            healthConditions = patientDetailsController.getHealthConditions().toArray() as String[];
          }
