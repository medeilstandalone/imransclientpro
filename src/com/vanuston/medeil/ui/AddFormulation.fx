...
public class AddFormulation {
    ...
    function FXinfo(msg: String): Void {
        CustomAlert.ShowInfo("Add formulation", msg);
        showAlertbox();
    }
    ...
    function saveformulation(): Void {
        try { 
         ...
         if(ms==null and ms==""){
              FXinfo("Please Enter Formulation ");
         }else if(m<=0){
              FXinfo("Please Enter Formulation ");

         }else if(ms.length()<3){
             FXinfo("Formulation Contains Minimum 3 Characters");
         }
         else{
              //var commonController:CommonImplements = new CommonController();
               var commonController: CommonImplements=RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
                 var a= commonController.getAddFormulation(ms);
                 if (a == 1) {
                    FXinfo("Formulation is Already Exists.");
